...
**Q1（基础）**：RAG 解决的核心问题是什么？为什么 LLM "知其然不知其所以然"？
> 给大模型外挂上下文，补充模型训练时不知道的知识（包括行业知识、实时信息等），同时通过检索+生成的机制减少幻觉并提供可溯源性。LLM 本质是预测模型，训练完成后知识固化，对于不知道的知识只能"夸夸其谈"（产生幻觉），因此需要外部非参数化知识来补充。

**Q2（理解）**：请解释"参数化知识"和"非参数化知识"的区别，以及 RAG 如何将它们结合？
> **参数化知识**：存储在模型权重中的知识。权重是神经网络中需要学习的数字参数，训练过程就是不断调整这些数字，让模型学会正确输出。训练完成后，知识以数字形式固化在模型内部 — 权重就是模型"记住"知识的方式。
>
> **非参数化知识**：存储在外部知识库中的知识，不在模型权重里。通过检索"找"出来，以 context 形式注入给 LLM。
>
> RAG 的结合方式：检索阶段从外部知识库获取非参数化知识 → 注入 context → LLM 结合自身参数化知识生成答案。
---

## 第二组：技术对比

**Q3（技术）**：Naive RAG 和 Advanced RAG 的核心区别是什么？Advanced RAG 在检索前后增加了哪些优化？


**Q4（应用）**：我现在的任务是让 AI 客服模仿特定明星的说话风格，应该选 RAG 还是微调？为什么？
选微调。说话风格是一种行为模式（语气、用词习惯、口头禅），不属于事实性知识，无法通过检索获得。RAG 擅长注入外部知识，微调才擅长学习风格和行为方式。客服模仿明星说话，本质是让模型学会"像他一样输出"，这是微调的专长。

---

## 第三组：深度思考

**Q5（辨析）**：如果模型因为训练数据截止日期是 2023 年，无法回答"今天天气怎么样"，RAG 能解决这个问题吗？为什么？

**Q6（综合）**：RAG 的"幻觉"问题和检索质量有什么关系？如何通过优化检索来减少幻觉？

---

## 第四组：进阶理解

**Q7（进阶）**：为什么说"RAG 已死"是个伪命题？文章用什么类比来说明这一点？

**Q8（实战）**：构建 RAG 系统时，分块策略（Chunking）为什么很重要？应该按什么原则来切分文档？
